...
def test_run_rpd_comparisons():
    ...
    for suite in ["test_files_229_FSEC", "test_files_229_PSD"]:
        test_dir = Path(__file__).parent / suite

        for test_case_dir in test_dir.iterdir():
            if not test_case_dir.is_dir():
                continue

            print(f"Validating {suite} test case:", test_case_dir.name)
